class Store:
    ...
    @classmethod
    def from_size(cls, name: str, type: str, capacity: int):
        size = capacity // 2
        new_instance = Store(name, type, size)
        return new_instance

    def add_item(self, item_name: str):

        if item_name not in Store.items.keys():
            Store.items[item_name] = 1
            return f"{item_name} added to the store"
        elif Store.items[item_name] < self.capacity:
            Store.items[item_name] += 1
            return f"{item_name} added to the store"
        return "Not enough capacity in the store"

    def remove_item(self, item_name: str, amount: int):
        if (item_name in Store.items.keys()) and (Store.items[item_name] >= amount):
