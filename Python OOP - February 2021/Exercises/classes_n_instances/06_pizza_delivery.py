...
class PizzaDelivery:
    ...
    def make_order(self) -> object:
        if PizzaDelivery.ordered:
            return self.is_ordered()
        PizzaDelivery.ordered = True
        ingredients_info = [f"{k}: {v}" for k, v in self.ingredients.items()]
        pizza_info = f"You've ordered pizza {self.name} prepared with " \
                     f"{', '.join(ingredients_info)} and the price will be {self.price:.1f}lv."
        return pizza_info


t = PizzaDelivery('Margarita', 12, {'cheese': 2, 'tomatoes': 1})
print(t.remove_ingredient('tomatoes', 2, 2))
